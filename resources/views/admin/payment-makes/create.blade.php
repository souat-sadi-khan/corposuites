<form class="ajax-form payment-make-form" method="POST" action="{{ route('admin.payment-makes.store') }}">
    <div class="modal-header fm-modal-head">
        <div>
            <h5 class="modal-title">Make Payment</h5>
            <p>Record a vendor payment and allocate it against one or more open invoices</p>
        </div>
    ...
    <div class="modal-body fm-modal-body fm-body">
        ...
        <p class="text-muted small mb-2">Select a vendor above first — only that vendor's open (unpaid/partially-paid) invoices will be selectable.</p>
        <div class="payment-make-item-rows"></div>
    ...
    {{-- Invoice options source, consumed client-side by payment-makes.js. Each option carries the owning vendor id and current balance due so rows can be filtered by the selected header vendor and pre-fill a sensible allocation amount. No per-row AJAX. --}}
