...
- [x] **Generate Payroll for All Employees (bulk generation)** — a "Generate for All" button next to the existing single-employee "Generate Payroll" button, opening a form with Month/Year plus six *optional* narrowing filters (Department, Designation, Shift, Employment Status, Employee Type, Gender — the same set already offered on the Employees advanced search) — leaving every filter blank targets every active employee company-wide. `PayrollService::bulkGenerate()` resolves the matching employee set and generates each one independently, skipping (never aborting the batch) an employee who: already has a payroll for that period, has no active salary structure, is on a commission-based structure (needs a per-employee sales figure a bulk run can't collect), or has a per-occurrence component (needs a per-employee occurrence count, same reason) — each skip is recorded with a plain-English reason on the Activity Log entry for that run, and the toast tells the admin how many were generated vs. skipped. Gated behind its own `payroll.bulk-generate` permission, separate from `payroll.create`, since a company-wide bulk run is a more sensitive action than generating one employee's payroll.
- [x] **Enterprise-Grade Expense Claim System** — requested as "the category is hand-written, I need dynamic options on master also the report, and add other relevant enterprise-grade features." Two clarifying questions (single-line vs. multi-line claims; which optional add-ons to build) were asked via the UI and went unanswered, so this was built on the stated **recommended defaults**: kept claims single-line (no line-item breakdown per claim — flagging this in case multi-line "expense reports" bundling several claims is wanted later), and added all three suggested enterprise add-ons (spending policy limits, reimbursement tracking, GL/Chart-of-Accounts mapping).
  - **Dynamic Expense Categories master** (new `expense_categories` table/screen, under Payroll & Finance) replacing the old hand-typed `category` string — each category carries an optional **Max Amount Per Claim** and **Receipt Required Above** spending-policy pair, plus an optional link to a postable (non-group) Chart of Accounts entry for GL mapping. The original `category` column was renamed to `category_legacy` (kept, nullable) and a one-time migration backfill auto-creates categories from any historical free-text values and links existing claims to them — so nothing pre-existing silently loses its category.
  - **Spending policy enforcement** — `ExpenseClaimRequest` now rejects a claim whose amount exceeds its category's configured max, and separately requires an uploaded (or already-on-file) receipt once the amount crosses the category's receipt threshold — both checks name the category and the exact limit in the error message.
  - **Reimbursement/payment tracking** — new `payment_status` (unpaid/paid), `payment_date`, `reimbursement_method` fields; a guarded "Mark Reimbursed" action (only available once a claim is `approved` and still `unpaid`, blocks a second reimbursement of the same claim) is exposed as a one-click icon on the list and as manual-correction fields on the edit form.
  - **Expense Claims Report** (new `admin.expense-claims-report.index` screen) — filterable by Category / Department / Employee / Approval State / Reimbursement State / date range, with stat tiles (total claims, total amount, pending, approved, rejected, amount awaiting reimbursement) and category/department breakdown tables — this is the concrete "also the report" half of the request, reading the same dynamic `ExpenseCategory` master the claim form now uses.
  - Verified end-to-end via a standalone PHP script exercising real DB round-trips (not raw model calls bypassing validation): category CRUD, over-max-amount rejection, missing-receipt-above-threshold rejection, a valid low-amount claim's acceptance, the pending→approved→reimbursed→double-reimbursement-rejected lifecycle, report-query aggregation, and confirming a deleted category's claims survive with `expense_category_id` cleared (not cascade-deleted) — all passed on the first clean run; every new/modified PHP file passed `php -l`; all touched routes resolve with the project's now-active `permission:` middleware convention.
