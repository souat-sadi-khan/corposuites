...
class HrmDetailExportController extends Controller {
 ...
 public function componentPrint(SalaryComponent $salaryComponent, Request $request){
    $rows=$this->componentRows($salaryComponent,$request)->get();
    $summary=[
        'total_entries' => $rows->count(),
        'total_amount' => $rows->sum('amount'),
    ...
    return view('admin.hrm-details.salary-component-print',compact('salaryComponent','rows','summary'));
 }
