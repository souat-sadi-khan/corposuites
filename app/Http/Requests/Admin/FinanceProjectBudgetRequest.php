...
class FinanceProjectBudgetRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            // project + period, not a global counter), the total is
            // summed from the lines.
            'project_id' => 'required|exists:projects,id',
            'title' => 'nullable|string|max:255',
            'period_type' => ['required', Rule::in(FinanceProjectBudget::PERIOD_TYPES)],
            'period_start' => 'required|date',
            'period_end' => 'required|date|after_or_equal:period_start',
            'budget_status' => ['required', Rule::in(FinanceProjectBudget::STATUSES)],
    ...
    /**
     * A project budget can only allocate planned spend to postable
     * (non-group) accounts — the same "no posting to group/header
     * accounts" rule Journal Entries/Budget Planning/Department Budget
     * already enforce, applied here to project budget lines.
     */
