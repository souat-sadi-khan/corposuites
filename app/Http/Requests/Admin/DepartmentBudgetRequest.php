...
class DepartmentBudgetRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            // department + period, not a global counter), the total is
            // summed from the lines.
            'department_id' => 'required|exists:departments,id',
            'title' => 'nullable|string|max:255',
            'period_type' => ['required', Rule::in(DepartmentBudget::PERIOD_TYPES)],
            'period_start' => 'required|date',
            'period_end' => 'required|date|after_or_equal:period_start',
            'budget_status' => ['required', Rule::in(DepartmentBudget::STATUSES)],
    ...
    {
        return [
            'department_id.required' => 'Select the department this budget belongs to.',
    ...
    /**
     * A department budget can only allocate planned spend to postable
     * (non-group) accounts — the same "no posting to group/header
     * accounts" rule Journal Entries/Budget Planning already enforce,
     * applied here to department budget lines.
     */
